...
from Facility_System_OLD__class import Facility
...
class Simulation:
    ...
    def simulate(self, T):
        ...
        t = 0
        fes = FES()                                                     # Initialize future event set
        ...
        while stop == False:
            ...
            if a2 > 0:
                stop = True
        p2 = Patient(a2, 2)                                             # Initialize first patient type 1
        ...
        while t < T:
            ...
            if e.type == Event.CALL:
                if p.patient_type == 1:
                    start_time = f1.schedule_facility(p, self.sched_scan_t1)                                    # Set start time to start time of scheduled appointment
                ...
                if p.patient_type == 2:
                    start_time = f2.schedule_facility(p, self.sched_scan_t2)                                    # Set start time to start time of scheduled appointment
                    sched = Event(Event.ARRIVAL, (self.Day_counter) * self.Duration_day + start_time, p)        # Create scheduled event
                    fes.add(sched)                                                                              # Add the event to the future event set
                ...
                if p.patient_type == 1:
                    ...
                    self.last_change_over_time_f1 = t                                                           # Update time of last change of status
                    if len(queue1.queue) >= 1:                                                                  # When still patients in the queue
                    ...
                    if len(queue2.queue) >= 1:                                                                  # When still people in the queue
                        ...
                        fes.add(dep)                                                                            # Add to Future event list
                        f2.status_busy = 1                                                                      # Change status of f2 to busy (1)
        return res                                                                                              # Return all values of the SimResults
...
def run_simulation(arrivalrate1, arrivalrate2, scantime1, scantime2, sched_scan_t1, sched_scan_t2, num_runs, limit):
    ...
    averaged_results.createhistogram(averaged_results.total_q1_time, 25, 'Old system: waiting time queue 1')
    averaged_results.createhistogram(averaged_results.total_q2_time, 25, 'Old system: waiting time queue 2')
    averaged_results.createhistogram(averaged_results.total_total_time, 25, 'Old system: waiting time both queues')
    averaged_results.createhistogram(averaged_results.total_time_overtime, 25, 'Old system: overtime')
...
print('Scheduling System: Old System')
